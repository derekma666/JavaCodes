...
Boolean：true、false，can use 1 bit to store。JVM will compile the boolean to int，in which 1 representes true，0 represents false. JVM support boolean arrays through read byte arrays.
...
- Integer.valueOf(123) will adopt the the object in cache, multiple-call will take the same citation of one object.
...
```java valueOf() ``` fistly determine the value is in the cache, if it is in the cache, it will directly return the content in cache. 
...
In Java 8, the range of Integer catch is -128\~127。
...
```
Compiler will automatically call valueOf(), therefore multiple variable with same values will be stored in the range of Interger in the cache, it will cite the same object.
...
```
Corresponding cache for basic types: 
